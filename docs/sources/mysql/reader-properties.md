...
Proper configuration of the MySQL server is necessary to use the MySQL adapter. The appropriate configuration steps vary depending on whether MySQL is being used [on-premises](/sources/mysql/mysql-server) or [Amazon RDS](/sources/mysql/amazon-rds). Please refer to the relevant sections for detailed instructions.
